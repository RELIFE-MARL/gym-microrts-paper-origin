...
class LocalAgentEnv(BaseSingleAgentEnv):
    def start_client(self):
        ...
        return JNIClient(
            rf,
            os.path.expanduser(self.config.microrts_path),
    ...
    def init_properties(self):
        ...
        self.observation_space = spaces.Box(
            low=0.0,
            high=1.0,
            shape=(self.config.height * self.config.width, sum(self.num_planes)),
            dtype=np.int32,
        )
    ...
    def _encode_obs(self, obs: List):
        ...
        obs_planes = np.zeros(
            (self.config.height * self.config.width, sum(self.num_planes)), dtype=np.int
        )
