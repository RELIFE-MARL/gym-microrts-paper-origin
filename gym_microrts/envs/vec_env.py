...
class MicroRTSVecEnv:
    ...
    def __init__(
        ...
        self.action_space = gym.spaces.MultiDiscrete(
            [self.height * self.width, 6, 4, 4, 4, 4, len(self.utt["unitTypes"]), 7 * 7]
        )
    ...
    def reset(self):
        ...
        raw_obs, reward, done, info = (
            np.array(responses.observation),
    ...
    def step_wait(self):
        responses = self.vec_client.step(
...
class MicroRTSGridModeVecEnv(MicroRTSVecEnv):
    ...
    def step_wait(self):
        ...
        raw_obs, reward, done, info = (
            ...
            np.array(responses.reward),
            np.array(responses.done),
            [{} for _ in range(self.num_selfplay_envs + self.num_envs)],
        )
        obs = []
        for ro in raw_obs:
            obs += [self._encode_obs(ro)]
