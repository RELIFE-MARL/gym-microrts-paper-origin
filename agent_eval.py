...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp-name",
        type=str,
        default="ppo_gridnet_diverse_encode_decode",
...
for i in range(len(ais)):
    if args.exp_name in gridnet_exps:
        ...
        envs = VecVideoRecorder(
            envs,
            f"videos/{experiment_name}/{ai_names[i]}",
...
if args.exp_name == "ppo_gridnet_diverse_impala":
    ...
    class Agent(nn.Module):
        def __init__(self, mapsize=16 * 16):
            ...
            self.actor = layer_init(
                nn.Linear(256, self.mapsize * envs.action_space.nvec[1:].sum()),
                std=0.01,
            )
    ...
    class Agent(nn.Module):
        def __init__(self, frames=4):
            ...
            self.network = nn.Sequential(*conv_seqs)
            self.actor = layer_init(
                nn.Linear(256, envs.action_space.nvec.sum()), std=0.01
            )
            self.critic = layer_init(nn.Linear(256, 1), std=1)
        ...
        def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
            ...
            if action is None:
                ...
                source_unit_mask = torch.Tensor(
                    np.array(envs.vec_client.getUnitLocationMasks()).reshape(1, -1)
                )
                multi_categoricals = [
                    CategoricalMasked(logits=split_logits[0], masks=source_unit_mask)
                ]
                ...
                source_unit_action_mask = torch.Tensor(
                    np.array(
                        envs.vec_client.getUnitActionMasks(
                            action_components[0].cpu().numpy()
                        )
                    ).reshape(1, -1)
                )
                split_suam = torch.split(
                    source_unit_action_mask, envs.action_space.nvec.tolist()[1:], dim=1
                )
                ...
                invalid_action_masks = torch.cat(
                    (source_unit_mask, source_unit_action_mask), 1
                )
                action_components += [
                    categorical.sample() for categorical in multi_categoricals[1:]
                ]
    ...
    class Agent(nn.Module):
        ...
        def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
            ...
            real_invalid_action_masks = real_invalid_action_masks.view(
                -1, 256, envs.action_space.nvec[1:].sum() + 1
            )
            return (
                action,
...
for envs_idx, envs in enumerate(ai_envs):
    ...
    while True:
        ...
        with torch.no_grad():
            action, logproba, _, invalid_action_mask = agent.get_action(
                next_obs, envs=envs
            )
        ...
        if args.exp_name in gridnet_exps:
            ...
            valid_actions = real_action[
                invalid_action_mask[:, :, 0].bool().cpu().numpy()
            ]
            valid_actions_counts = (
                invalid_action_mask[:, :, 0].sum(1).long().cpu().numpy()
            )
            java_valid_actions = []
            valid_action_idx = 0
        ...
        if game_count >= args.num_eval_runs:
            ...
            for (label, val) in zip(
                ["loss", "tie", "win"], ai_match_stats[ai_names[envs_idx]]
            ):
                writer.add_scalar(f"charts/{ai_names[envs_idx]}/{label}", val, 0)
            if args.prod_mode and args.capture_video:
                video_files = glob.glob(
                    f"videos/{experiment_name}/{ai_names[envs_idx]}/*.mp4"
                )
                for video_file in video_files:
                    print(video_file)
                    wandb.log(
                        {
                            f"RL agent against {ai_names[envs_idx]}": wandb.Video(
                                video_file
                            )
                        }
                    )
...
cumulative_match_results_rate = (
    cumulative_match_results / cumulative_match_results.sum()
)
if args.prod_mode:
    ...
    for (label, val) in zip(
        ["loss rate", "tie rate", "win rate"], cumulative_match_results_rate
    ):
        writer.add_scalar(f"charts/cumulative_match_results/{label}", val, 0)
