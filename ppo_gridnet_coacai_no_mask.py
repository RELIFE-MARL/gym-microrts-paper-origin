...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16):
        ...
        self.actor = layer_init(
            nn.Linear(256, self.mapsize * envs.action_space.nvec[1:].sum()), std=0.01
        )
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            ...
            (
                ...
                logproba,
                _,
                invalid_action_masks[step],
                real_invalid_action_masks,
            ) = agent.get_action(obs[step], envs=envs)
        ...
        valid_actions = real_action[
            real_invalid_action_masks[:, :, 0].bool().cpu().numpy()
        ]
        valid_actions_counts = (
            real_invalid_action_masks[:, :, 0].sum(1).long().cpu().numpy()
        )
        java_valid_actions = []
        valid_action_idx = 0
